		...
		>
			<WrapperForm title='Thông tin bài đăng'>
				<FlexLayout column gap-16>
					...
					<GridColumn col_3>
						<InfoDetail
							name='Giá điện/kiểu tính'
							value={
								detailAdvertisement?.adElectricInfo
									? `${detailAdvertisement.adElectricInfo.price.toLocaleString('vi-VN')}/${getUnitByAdPrice(
											detailAdvertisement.adElectricInfo.serviceUu?.type ?? 0,
											detailAdvertisement.adElectricInfo.type ?? 0
									  )}`
									: '---'
							}
						/>
						<InfoDetail
							name='Giá nước/kiểu tính'
							value={
								detailAdvertisement?.adWaterInfo
									? `${detailAdvertisement.adWaterInfo.price.toLocaleString('vi-VN')}/${getUnitByAdPrice(
											detailAdvertisement.adWaterInfo.serviceUu?.type ?? 0,
											detailAdvertisement.adWaterInfo.type ?? 0
									  )}`
									: '---'
							}
						/>
						<InfoDetail name='Chỉnh sửa gần nhất' value={'---'} />
