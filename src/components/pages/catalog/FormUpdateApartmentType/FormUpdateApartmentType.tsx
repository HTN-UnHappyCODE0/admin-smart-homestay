...
import styles from './FormUpdateApartmentType.module.scss';
import {PropsFormUpdateApartmentType} from './interfaces';
...
function FormUpdateApartmentType({onClose}: PropsFormUpdateApartmentType) {
	...
	useQuery<{name: string; description: string; id: number; uuid: string; status: number}>(
		[QUERY_KEY.detail_apartment_type, _uuidUpdate],
		{
			queryFn: () =>
				httpRequest({
					http: apartmentTypeServices.detailApartmentType({
	...
	const funcUpdateApartmentType = useMutation({
		mutationFn: () =>
			httpRequest({
				showMessageFailed: true,
				showMessageSuccess: true,
				msgSuccess: 'Chỉnh sửa loại hình căn hộ thành công!',
				http: apartmentTypeServices.updateApartmentType({
					uuid: _uuidUpdate as string,
					name: form.name,
...
export default FormUpdateApartmentType;
