...
import styles from './MainMeterType.module.scss';
import {IMeterType, PropsMainMeterType} from './interfaces';
...
import FormCreateMeterType from '../FormCreateMeterType';
import FormUpdateMeterType from '../FormUpdateMeterType';

function MainMeterType({}: PropsMainMeterType) {
	...
	}>([QUERY_KEY.table_meter_type, page, pageSize, keyword, status], {
		queryFn: () =>
			httpRequest({
			...
			return httpRequest({
				...
				msgSuccess:
					dataChangeStatus?.status == STATUS_CONFIG.ACTIVE
						? 'Khóa loại thiết bị thành công!'
						: 'Mở khóa loại thiết bị thành công!',
	...
	return (
		<Fragment>
										...
										render: (row, _) => (
											<FlexLayout row>
												<IconActionTable
													...
													tooltip={
														row?.status == STATUS_CONFIG.ACTIVE ? 'Khóa loại thiết bị' : 'Mở khóa loại thiết bị'
													}
													onClick={() =>
														setDataChangeStatus({
															uuid: row?.uuid,
												...
												<IconActionTable
													...
													tooltip='Chỉnh sửa loại thiết bị'
													onClick={() =>
														router.replace({
															pathname: router.pathname,
															query: {
			...
			<Dialog
				...
				title={dataChangeStatus?.status == STATUS_CONFIG.ACTIVE ? 'Khoá loại thiết bị' : 'Mở khóa loại thiết bị'}
				note={
					dataChangeStatus?.status == STATUS_CONFIG.ACTIVE
						? 'Bạn có chắc chắn muốn khóa loại thiết bị không?'
						: 'Bạn có chắc chắn muốn mở khóa loại thiết bị không?'
				}
				icon={
					dataChangeStatus?.status == STATUS_CONFIG.ACTIVE ? (
